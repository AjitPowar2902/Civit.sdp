...
const apiService = GlobalServiceFile(
  CreateAxiosInstance(process.env.REACT_APP_PLOTSERVICE_URL)
);
...
export default {
  ...
  getSubLettee: async (plotId) => {
    ...
    if (response.data.IsSuccess == true) {
      console.log("from plotservice",response.data);
      result = response.data;
    } else {
      console.log("from plotservice error",response.data);
      result = null;
    }
    return result;
  },
